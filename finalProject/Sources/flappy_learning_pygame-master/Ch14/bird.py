...
class Bird():
    ...
    def assign_collision_fitness(self, p): #if a bird has collided with a pipe, check how close the nearest gap was
        gap_y = 0
        if p.pipe_type == PIPE_UPPER: #it's different if it was a top pipe and a bottom pipe
            gap_y = p.rect.bottom + PIPE_GAP_SIZE / 2
        else:
            gap_y = p.rect.top - PIPE_GAP_SIZE / 2

        self.fitness = -(abs(self.rect.centery - gap_y)) #Higher is worse, because it's further, so we make it a negative absolute value
...
class BirdCollection():
    ...
    def evolve_population(self): #Go through every bird and give it a fitness score

        for b in self.birds: #for every bird
            b.fitness += b.time_lived * PIPE_SPEED #add to it's fitness the time lived number

        self.birds.sort(key=lambda x: x.fitness, reverse=True) #sort the birds by fitness

        for b in self.birds[0:10]: #print their fitness
            print('fitness:', b.fitness)

        self.create_new_generation() #then make a new generation
