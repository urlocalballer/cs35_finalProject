...
class BirdCollection(): #adding new bird collection
    ...
    def create_new_generation(self):
        self.birds = []
        for i in range(0, GENERATION_SIZE): #generation size is number of birds created at once by neural network
            self.birds.append(Bird(self.gameDisplay))

    def update(self, dt, pipes):
        num_alive = 0   #assume no birds are alive
        for b in self.birds:    #every single bird will
            if random.randint(0,4) == 1:    #either randomly jump or fall
                b.jump()
            b.update(dt, pipes) #update its position and
            if b.state == BIRD_ALIVE: #if its alive,
                num_alive += 1 #we know that one more is alive
