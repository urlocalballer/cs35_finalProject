...
def run_game():
    ...
    while running:
        ...
        pipes.update(dt)
        num_alive = birds.update(dt, pipes.pipes)   #check how many birds from the beginning still live

        if num_alive == 0:  #if they are all dead
            pipes.create_new_set()  #reset the pipes
            game_time = 0   #reset the timer
            birds.create_new_generation()   #reset the birds
            num_iterations += 1 #and do it again
