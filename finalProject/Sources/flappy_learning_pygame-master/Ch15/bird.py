...
class Bird():
    ...
    def create_offspring(p1, p2, gameDisplay): #makes a baby bird
        new_bird = Bird(gameDisplay)
        new_bird.nnet.create_mixed_weights(p1.nnet, p2.nnet) #mixes the neural nets so that their baby interprets data kinda like the both
        return new_bird
...
class BirdCollection():
    ...
    def evolve_population(self):
        ...
        cut_off = int(len(self.birds) * MUTATION_CUT_OFF) #how many of the good birds from this generation we want to keep
        good_birds = self.birds[0:cut_off] #these are the best birds from this generation
        bad_birds = self.birds[cut_off:] #these are the rest
        num_bad_to_take = int(len(self.birds) * MUTATION_BAD_TO_KEEP) #but we take some of the bad birds

        for b in bad_birds: #modifies the weights of the shitty birds for some variation
        ...
        while len(new_birds) < len(self.birds): #while we still need birds
            idx_to_breed = np.random.choice(np.arange(len(good_birds)), 2, replace=False) #take the index of two good birds to breed
            if idx_to_breed[0] != idx_to_breed[1]: #make sure we don't just make the same bird again
                new_bird = Bird.create_offspring(good_birds[idx_to_breed[0]], good_birds[idx_to_breed[1]], self.gameDisplay) #new baby bird
                if random.random() < MUTATION_MODIFY_CHANCE_LIMIT: #and maybe we mutate the baby bird too
                    new_bird.nnet.modify_weights()
                new_birds.append(new_bird) #then add it to the list

        for b in new_birds: #reset every new bird
            b.reset();

        self.birds = new_birds #and then make this the new new_birds
